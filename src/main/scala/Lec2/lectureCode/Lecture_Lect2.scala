...
object IfElse extends App {
  ...
  if (x == y) f()
  // Фигурные скобки обозначают многострочный код, можно писать код в несколько строчек без ";"
  if (x == y) {
    f()
  }

  // If c несколькими условями
  if (x == y) {
    f()
  } else if (y == z) {
    g()
  ...
  val res2: Unit = if (10 < 2) println("Да") else println("Нет")

}

/**
 * Lazy метод не вычисляет результат до тех пор, пока этот результат
не будет как-то использоваться, например, выводиться на печать.
 */
...
object For extends App {
  ...
  val line = for (i <- 1 to 50) yield i
...
object Rec extends App {
  ...
  println(factorial(BigInt(15))) // ce
}
